...
export class A3Reviewer {
  ...
  public async reviewAllTasks(payload: {
    ...
    const prompt = [
      ...
      '',
      '### DESIGN, UX, AND QUALITY BAR (reject anything that fails):',
      'Judge the delivered product the way a demanding senior product designer would.',
      'Users must WANT to use it and come back. Check for:',
      '- VISUAL POLISH: consistent typography, spacing, alignment, and color hierarchy; deliberate visual style that fits the product.',
      '- UX QUALITY: obvious primary actions, clear feedback for every interaction (hover, active, loading, error, empty states), intuitive flows, no dead ends.',
      '- AI SLOP IS AN AUTOMATIC REJECT. Red flags include:',
      '  * generic gradient purple/blue landing pages with no product identity',
      '  * emoji-stuffed headings or buttons used instead of real iconography/copy',
      '  * lorem ipsum, placeholder text, TODO comments, or fake/sample data presented as real',
      '  * identical generic card grids with no information hierarchy',
      '  * stock phrases like "Welcome to X! Get started today!" with no substance',
      '  * inconsistent component styling (different radii, shadows, fonts per screen)',
      '- DELIGHT: at least one memorable detail (micro-animation, satisfying interaction, personality in copy) — nothing that feels auto-generated.',
      '- If the product is a game or interactive experience: is it actually FUN and readable at a glance? Clunky controls or unclear goals = reject.',
      'When a task fails the quality bar, list it in rejectedTaskIds with a concrete blockingIssue describing exactly what looks or feels cheap and what to change.',
      '',
      '### TESTS:',
      payload.testSummary,
      '',
